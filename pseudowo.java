...
public class pseudowo implements MouseListener {

    public JFrame frame;
    public JPanel canvasPanel;
    public JPanel controlPanel;
    public ArrayList<JPPanel> rooms = new ArrayList<JPPanel>();
    ...
    public pseudowo() {
        ...
        floor.setBounds(70, 70, (((frameWidth - 30) * 3) / 4) - 140, (frameHeight - 50) -140 + 8);
    ...
    public void OpenFile() {
        ...
        if (option == JFileChooser.APPROVE_OPTION) {
            ...
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                ArrayList<JPPanel> serializablePanels = (ArrayList<JPPanel>) ois.readObject();
                rooms.clear();
                floor.removeAll();

                for (JPPanel sp : serializablePanels) {
                    floor.add(sp);
                    rooms.add(sp);
                    // roomNum++;
                }
    ...
    public void Addroom(int l, int b, String roomType) {
        xCoord=0;
        yCoord=0;
        rooms.add(new JPPanel(rooms,floor));
        rooms.get(rooms.size()-1).setBounds(xCoord, yCoord, b, l);
        while(true){
        if(!OverlapCheck(rooms.get(rooms.size()-1))){
        ...
        switch (roomType) {
            case "Bedroom":
                rooms.get(rooms.size()-1).setBackground(new Color(0x99DD5C)); // Green
                break;
            case "Bathroom":
                rooms.get(rooms.size()-1).setBackground(new Color(0x0050FF)); // Blue
                break;
            case "Kitchen":
                rooms.get(rooms.size()-1).setBackground(new Color(0xFF1515)); // Red
                break;
            case "Dining Room":
                rooms.get(rooms.size()-1).setBackground(new Color(0xFF7700)); // Orange
                break;
            case "Drawing Room":
                rooms.get(rooms.size()-1).setBackground(Color.YELLOW); // Yellow
                break;
        }

        rooms.get(rooms.size()-1).setBorder(BorderFactory.createLineBorder(Color.black, 2));
        floor.add(rooms.get(rooms.size()-1), Integer.valueOf(2));
        // roomNum++;
